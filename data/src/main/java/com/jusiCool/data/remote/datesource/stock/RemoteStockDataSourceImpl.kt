...
class RemoteStockDataSourceImpl @Inject constructor(
    ...
    override suspend fun getStockDetail(stockId: Long): Flow<GetStockDetailResponse> =
    ...
    override suspend fun buyStock(stockId: Long, body: StockRequest): Flow<Unit> =
        performApiRequest { serviceStock.buyStock(
            stockId = stockId,
            body = body
        ) }

    override suspend fun sellStockReserve(stockId: Long, body: BuyStockRequest): Flow<Unit> =
        performApiRequest { serviceStock.sellStockReserve(
            stockId = stockId,
            body = body
        ) }

    override suspend fun buyStockReserve(stockId: Long, body: BuyStockRequest): Flow<Unit> =
